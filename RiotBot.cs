            ...
            else if (!(message is GameNotification) && !(message is SearchingForMatchNotification))
            {
                if (message is EndOfGameStats)
                {
                    ...
                    else
                    {
                        ...
                        {
                            this.updateStatus("Xin vui lòng chờ trong: " + (float)(this.m_leaverBustedPenalty / 1000) / 60f + " phút! Để thoát hàng chờ thấp", this.Accountname);
                            ...
                            if (m.PlayerJoinFailures == null)
                            {
                                this.updateStatus("Thoát hàng chờ thấp thành công LOL!", this.Accountname);
                            }
                            else
                            {
